function my_oef_vsm(dhn_regulation_mode, num_iter)
...
model_building();
model_couple();
model_regulation_strategy(dhn_regulation_mode, num_iter);
...
% model.ies_vsm.cons = model.ies_vsm.cons + (( ...
%     model.ies_vsm.var.dhn.h_load(t, indexset_loadnode) == ...
%     1e3 * model.oef.var.dhn.h_load(t, indexset_loadnode) ...
%     ));
...
%% regulation strategy
function model_regulation_strategy(dhn_regulation_mode, num_iter)
...
% % gen
% model.ies_vsm.var.eps.P_gen_bin = sdpvar(num_period, num_gen, 'full');
% model.ies_vsm.var.eps.U_gen_aux_pos = sdpvar(num_period, num_gen, 'full');
% model.ies_vsm.var.eps.U_gen_aux_neg = sdpvar(num_period, num_gen, 'full');
% % chp
% model.ies_vsm.var.eps.s_pos = sdpvar(num_period, num_gen, 2, 'full');
% model.ies_vsm.var.eps.s_neg = sdpvar(num_period, num_gen, 2, 'full');
% model.ies_vsm.var.eps.v_pos = sdpvar(num_period, num_gen, 2, 'full');
% model.ies_vsm.var.eps.v_neg = sdpvar(num_period, num_gen, 2, 'full');
% model.ies_vsm.var.eps.P_gen_aux = sdpvar(num_period, num_gen, 2, 'full');
% model.ies_vsm.var.eps.P_gen_aux0 = sdpvar(num_period, num_gen, 'full');
% % dhn


%% voltage security margin
%% vsm: bus voltage & Q_gen of generator
% model.ies_vsm.cons = model.ies_vsm.cons + (( ...
%     model.ies_vsm.var.eps.U_gen_aux_pos(:) >= 0) : 'U_gen_aux >= 0');
% model.ies_vsm.cons = model.ies_vsm.cons + (( ...
%     model.ies_vsm.var.eps.U_gen_aux_neg(:) >= 0) : 'U_gen_aux >= 0');
% 
% model.ies_vsm.cons = model.ies_vsm.cons + (( ...
%     model.ies_vsm.var.eps.U(:, indexset_gen(2:end)) == ...
%     model.oef.var.eps.U(:, indexset_gen(2:end)) + ...
%     model.ies_vsm.var.eps.U_gen_aux_pos(:, 2:end) - ...
%     model.ies_vsm.var.eps.U_gen_aux_neg(:, 2:end)) : 'U = U0 + U_gen_aux_pos - U_gen_aux_neg');
% 
% model.ies_vsm.cons = model.ies_vsm.cons + (( ...
%     (ones(num_period, 1) * data.eps.gen(2:end,4)' - ...
%     model.ies_vsm.var.eps.Q_gen(:, 2:end)) .* ...
%     model.ies_vsm.var.eps.U_gen_aux_neg(:, 2:end) == 0) : '(Q_upper - Q_gen) * U_gen_aux_neg == 0');
% 
% model.ies_vsm.cons = model.ies_vsm.cons + (( ...
%     (model.ies_vsm.var.eps.Q_gen(:, indexset_gen(2:end)) - ...
%     ones(num_period, 1) * data.eps.gen(2:end,5)') .* ...
%     model.ies_vsm.var.eps.U_gen_aux_pos(:, 2:end) == 0) : '(Q_gen - Q_gen) * U_gen_aux_pos == 0');

%% vsm: P of generator
% indexset_PV = setdiff(indexset_gen, union(indexset_chp, 1));
% model.ies_vsm.cons = model.ies_vsm.cons + (( ...
%     model.ies_vsm.var.eps.P_gen(:, indexset_PV) <= ...
%     (1 + model.ies_vsm.var.lambda_e) * ...
%     model.oef.var.eps.P_gen(:, indexset_PV)) : 'P_gen <= (1+lambda) * P_gen');
% model.ies_vsm.cons = model.ies_vsm.cons + (( ...
%     model.ies_vsm.var.eps.P_gen(:, indexset_PV) <= ...
%     ones(num_period, 1) * data.eps.gen(indexset_PV,9)') : 'P_gen <= P_upper');
% 
% model.ies_vsm.cons = model.ies_vsm.cons + (( ...
%     model.ies_vsm.var.eps.P_gen(:, indexset_PV) >= ...
%     (1 + model.ies_vsm.var.lambda_e) * ...
%     model.oef.var.eps.P_gen(:, indexset_PV) - ...
%     data.settings.big_M * ...
%     model.ies_vsm.var.eps.P_gen_bin(:, indexset_PV)) : 'P_gen >= (1+lambda) * P_gen + M * binary');
% model.ies_vsm.cons = model.ies_vsm.cons + (( ...
%     model.ies_vsm.var.eps.P_gen(:, indexset_PV) >= ...
%     ones(num_period, 1) * data.eps.gen(indexset_PV,9)' - ...
%     data.settings.big_M * ...
%     (1 - model.ies_vsm.var.eps.P_gen_bin(:, indexset_PV))) : 'P_gen >= P_upper - M * (1-binary)');
% 
% model.ies_vsm.cons = model.ies_vsm.cons + (( ...
%     0 <= model.ies_vsm.var.eps.P_gen_bin <= 1) : '0 <= binary <= 1');
% model.ies_vsm.cons = model.ies_vsm.cons + (( ...
%     model.ies_vsm.var.eps.P_gen_bin == model.ies_vsm.var.eps.P_gen_bin .^2) : 'enforce to be binary');

%% vsm: CHP
% for i = 1 : num_gen
%     if data.eps.gen(i,24) == 2       
%         for j = 1 : 2
%             model.ies_vsm.cons = model.ies_vsm.cons + (( ...
%                 ones(num_period,1) * data.eps.gen_feasibleregion(i,1).c(j,1) - ...
%                 (data.eps.gen_feasibleregion(i,1).a(j,1) * ...
%                 (1 + model.ies_vsm.var.lambda_e) * ...
%                 model.oef.var.eps.P_gen(:,i) + ...
%                 data.eps.gen_feasibleregion(i,1).b(j,1) * ...
%                 model.ies_vsm.var.eps.H_gen(:,i)) == ...
%                 model.ies_vsm.var.eps.s_pos(:,i,j) - ...
%                 model.ies_vsm.var.eps.s_neg(:,i,j) ...
%                 ): 'c - (a*(1+lambda)*P + b*H) = s+ - s-');
% 
%             model.ies_vsm.cons = model.ies_vsm.cons + (( ...
%                 0 <= model.ies_vsm.var.eps.s_pos(:,i,j) <= ...
%                 data.settings.big_M * ...
%                 model.ies_vsm.var.eps.v_pos(:,i,j) ...
%                 ) : '0 <= s_pos <= M * v_pos');
%             model.ies_vsm.cons = model.ies_vsm.cons + (( ...
%                 0 <= model.ies_vsm.var.eps.s_neg(:,i,j) <= ...
%                 data.settings.big_M * ...
%                 model.ies_vsm.var.eps.v_neg(:,i,j) ...
%                 ) : '0 <= s_neg <= M * v_neg');
% 
%             model.ies_vsm.cons = model.ies_vsm.cons + (( ...
%                 model.ies_vsm.var.eps.v_pos(:,i,j) == ...
%                 model.ies_vsm.var.eps.v_pos(:,i,j) .^2 ...
%                 ) : 'v_pos = v_pos ^2');
%             model.ies_vsm.cons = model.ies_vsm.cons + (( ...
%                 model.ies_vsm.var.eps.v_neg(:,i,j) == ...
%                 model.ies_vsm.var.eps.v_neg(:,i,j) .^2 ...
%                 ) : 'v_neg = v_neg ^2');
%             model.ies_vsm.cons = model.ies_vsm.cons + (( ...
%                 model.ies_vsm.var.eps.v_pos(:,i,j) + ...
%                 model.ies_vsm.var.eps.v_neg(:,i,j) == 1 ...
%                 ) : 'v_pos + v_neg = 1');
% 
%             model.ies_vsm.cons = model.ies_vsm.cons + (( ...
%                 ones(num_period,1) * data.eps.gen_feasibleregion(i,1).c(j,1) == ...
%                 data.eps.gen_feasibleregion(i,1).a(j,1) * ...
%                 model.ies_vsm.var.eps.P_gen_aux(:,i,j) + ...
%                 data.eps.gen_feasibleregion(i,1).b(j,1) * ...
%                 model.ies_vsm.var.eps.H_gen(:,i) ...
%                 ): '(a*P_aux + b*H) = c');
%         end
% 
%         model.ies_vsm.cons = model.ies_vsm.cons + (( ...
%             model.ies_vsm.var.eps.P_gen_aux0(:,i) == ...
%             (1 + model.ies_vsm.var.lambda_e) * ...
%             model.oef.var.eps.P_gen(:,i) ...
%             ): 'P_aux0 = (1 + lambda) * P0');
% 
%         model.ies_vsm.cons = model.ies_vsm.cons + (( ...
%             model.ies_vsm.var.eps.P_gen(:,i) == ...
%             (1 - model.ies_vsm.var.eps.v_neg(:,i,1) - ...
%             model.ies_vsm.var.eps.v_neg(:,i,2)) .* ...
%             model.ies_vsm.var.eps.P_gen_aux0(:,i) + ...
%             sum(model.ies_vsm.var.eps.v_neg(:,i,:) .* ...
%             model.ies_vsm.var.eps.P_gen_aux(:,i,:), 3) ...
%             ): 'P = (1 - v_neg(1) - v_neg(2)) * P_aux0 + sum(v_neg * P_aux)');
% 
%     end
% end
...
% model.ies_vsm.cons = model.ies_vsm.cons + (( ...
%     model.ies_vsm.var.lambda_e >= 0 ...
%     ) : '');

% model.ies_vsm.cons = model.ies_vsm.cons + (( ...
%     model.ies_vsm.var.lambda_h >= 0 ...
%     ) : '');


%% voltage security margin
% for i = 1 : size(data.eps.gen, 1)
%     if data.eps.gen(i, 24) >= 2
%         nodeID = data.eps.gen(i,5);
%         model.ies_vsm.cons = model.ies_vsm.cons + (( ...
%             ( - ...
%             model.ies_vsm.var.dhn.T_pipe_s_in(:, nodeID)) .* ...
%             (data.eps.gen_feasibleregion(i).Hmax - ...
%             model.ies_vsm.var.eps.H_gen(:, i)) == 0 ...
%             ) : '(Tset - Ts)*(Hmax - H) = 0');
%     end
% end
% for i = 1 : size(data.dhn.device, 1)
%     nodeID = data.dhn.device(i, data.loc.dhn.device.nodeID);
%     model.ies_vsm.cons = model.ies_vsm.cons + (( ...
%         ( - ...
%         model.ies_vsm.var.dhn.T_pipe_s_in(:, nodeID)) .* ...
%         (data.eps.gen_feasibleregion(i).Hmax - ...
