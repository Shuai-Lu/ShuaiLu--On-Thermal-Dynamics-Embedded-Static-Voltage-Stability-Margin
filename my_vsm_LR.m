function my_vsm_LR()
% voltage security margin based on linear regulation strategy of generation
%
%
% by Shuai Lu
...
model.vsm.cons = model.vsm.cons + (( ...
    ...
    ones(num_period, 1) * data.eps.gen(2:end,4)') : 'Q_lower <= Q_gen <= Q_upper');
...
model.vsm.cons = model.vsm.cons + (( ...
    model.vsm.var.P_gen(:, indexset_gen(2:end)) <= ...
    ones(num_period, 1) * data.eps.gen(2:end,9)') : 'P_gen <= P_upper');
...
model.vsm.cons = model.vsm.cons + (( ...
    model.vsm.var.P_gen(:, indexset_gen(2:end)) >= ...
    ones(num_period, 1) * data.eps.gen(2:end,9)' - ...
    data.settings.big_M * ...
...
model.vsm.cons = model.vsm.cons + (( ...
    ...
    (1 + model.vsm.var.lambda) * ...
    data.profile.Q_load(1:num_period, indexset_load)) : 'Q load increase pattern');
